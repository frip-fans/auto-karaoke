...
仓库保存代码、文档、配置模板、自造测试数据，以及用户指定的 [三张 README 示例截图](images/README.md)。截图按文件路径与 SHA-256 单独列入白名单；其他歌曲音视频、分轨、真实歌词文件、识别文本、成品字幕、模型权重和运行缓存不进入 Git。 `.gitignore` 和文本文件白名单检查用于降低误提交风险；新文档仍需人工检查，脚本不能判断一段文字的版权来源。
